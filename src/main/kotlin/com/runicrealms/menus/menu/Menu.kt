...
import com.runicrealms.menus.util.NumberUtil
...
/**
 * Represents a menu that is created and handles logic for GUI elements and player interaction. To
 * open the menu, use openMenu(Player).
 *
 ...
 * @property items The items this menu contains, mapping between slots and an ItemStack wrapper,
 *   MenuItem
 * @property size The inventory size for this menu, size must be between 9 and 54 and be a multiple
 *   of 9
 * @property animations List of menu animations to run when a player opens this menu
 * @property interactionsBlocked Default value deciding if to allow players to move items in the
...
constructor(
    ...
    internal var inventory: Inventory =
        if (title.content().isEmpty() || title.content().isBlank())
    ...
    init {
        ...
        if (items.size > 54 || items.size > size)
            throw IllegalArgumentException("Menu cannot have more items than determined GUI size!")

        // Fill the inventory
        for ((slot, item) in items) inventory.setItem(slot, item.icon)
        // For all of the menu items in the inventory that don't have interactionsBlocked set, apply
        // this menu's default
        ...
        for (animation in animations) {
            if (!sortedAnimations.containsKey(animation.interval))
                sortedAnimations[animation.interval] = LinkedList()
            sortedAnimations[animation.interval]!!.add(animation)
        }
        // This is the interval we will use as the lowest possible interval to cover all animation
        // intervals
     ...
     *
     * Gets the inventory that backs this menu. This should be used for observation and no code
     * should modify this inventory.
     */
    @Warning(reason = "Do not use for opening this menu") override fun getInventory() = inventory
    ...
    /**
     * Opens this menu for a player. This will not only send packets to the player with the
     * inventory but also handle certain things server side to keep the menu working.
     *
    ...
    /**
     * Wrapper for the menu animations currently running for a viewer of this menu. Each instance is
     * dynamically tied to an instance of a menu. RunningAnimations objects become useless after
     * they have been started and stopped for a viewer.
     *
    ...
    internal inner class RunningAnimations(val player: Player) {
        ...
        fun start() {
            if (task != null || hasRun)
                throw IllegalStateException(
                    "Cannot start RunningAnimations because it has already been run before!"
                )
            task =
                Bukkit.getScheduler()
                    .runTaskTimer(
                        ...
                        Runnable {
                            for (animation in intervalCounter.keys) {
                                intervalCounter[animation] =
                                    intervalCounter[animation]!! + animationInterval
                                if (intervalCounter[animation]!! >= animation.interval) {
                                    intervalCounter[animation] =
                                        intervalCounter[animation]!! - animation.interval
        ...
        fun stop() {
            task
                ?: throw IllegalStateException(
                    "Cannot stop RunningAnimations because it has not been started!"
                )
