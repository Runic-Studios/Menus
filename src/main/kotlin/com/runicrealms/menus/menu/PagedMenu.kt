 ...
 *
 * Size (from the super class) refers to the size of the first page only. Menu#items and
 * Menu#inventory will contain the first page of this menu. Title is the title of the first page
 * only.
 *
 ...
 * @param interactionsBlocked Default value deciding if to allow players to move items in the
 *   inventory. Can be overwritten by MenuItems.
...
constructor(
    ...
    Menu(
        menusAPI,
        pages.firstOrNull()?.title
            ?: throw IllegalArgumentException("PagedMenu must have at least one page!"),
        pages.firstOrNull()?.items
            ?: throw IllegalArgumentException("PagedMenu must have at least one page!"),
        pages.firstOrNull()?.size
            ?: throw IllegalArgumentException("PagedMenu must have at least one page!"),
        animations,
        interactionsBlocked,
        onClickHandler,
    ...
    init {
        ...
        if (pages.size > 1)
            for (pageNumber in 1 until pages.size) {
                MenuUtil.applyDefaultInteractionsBlocked(
        ...
        if (pages.size > 1)
            for (pageNumber in 1 until pages.size) {
                ...
                val pageInventory =
                    if (
                        currentPage.title.content().isEmpty() ||
                            currentPage.title.content().isBlank()
                    )
                        Bukkit.createInventory(this, currentPage.size)
     ...
     *
     * Gets the first page's inventory that backs this menu. This should be used for observation and
     * no code should modify this inventory. Use getInventoryPage(Int) to get a specific page.
     */
     ...
     * API note: WARNING - Do not use this to open a menu, instead use Menu#openMenu!
     *
     * Gets a specific page's inventory that backs it. This should be used for observation and no
     * code should modify this inventory.
     */
    fun getInventoryPage(page: Int): Inventory {
        if (!isValidPage(page))
            throw IllegalArgumentException("Page $page does not exist in this menu!")
    ...
    fun getMenuItem(slot: Int, page: Int): MenuItem {
        if (!containsMenuItem(slot, page))
            throw IllegalArgumentException(
                "Menu does not contain item at slot $slot! Use containsMenuItem to check first."
            )
    ...
    /**
     * Opens the first page of this menu for a player. This will not only send packets to the player
     * with the inventory but also handle certain things server side to keep the menu working.
     *
    ...
    /**
     * Opens a specific page of this menu for a player. This will not only send packets to the
     * player with the inventory but also handle certain things server side to keep the menu
     * working.
     *
     * @param player Menu viewer
    ...
    /**
     * Turns to the next page for a viewer of this menu. The player given must be an active viewer.
     *
     * @param player Menu viewer
     */
    fun openNextPage(player: Player) {
        if (!viewerPages.containsKey(player.uniqueId))
            throw IllegalStateException(
                "Cannot open next page for PagedMenu if player is not viewing this menu!"
            )
    ...
    /**
     * Turns to the previous page for a viewer of this menu. The player given must be an active
     * viewer.
     *
     * @param player Menu viewer
     */
     ...
     */
    class Page(var title: TextComponent, items: MutableMap<Int, MenuItem>, size: Int) {
        ...
        constructor(title: TextComponent, size: Int) : this(title, HashMap<Int, MenuItem>(), size)

        init {
            if (size !in 0..54 || size % 9 != 0)
                throw IllegalArgumentException("MenuPageItems invalid page size: $size")
        }

        /**
         * Sets an item icon in this page. Note that if the slot doesn't fit in the inventory's
         * current size, the inventory will be expanded.
         *
        ...
        /**
         * Checks if this inventory is full, meaning that if we didn't expand the inventory at all,
         ...
         * @see com.runicrealms.menus.util.MenuUtil.getFirstEmptySlot
         */
