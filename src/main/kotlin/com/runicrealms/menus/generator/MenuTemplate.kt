package com.runicrealms.menus.generator
 ...
 * This is used primarily for generating customized menus for each player, where some menu items are
 * static (same for all players) and some change per player.
 *
 * The type parameter for this class represents the type of MenuBuilder that will be created and use
 * to build menus.
 *
 * The initialization of the menu builder and dynamic elements is set to be lazy by default (called
 * on demand), but can be modified using a MenuGeneratorBuilder.
 *
 ...
 * @see com.runicrealms.menus.generator.MenuTemplateBuilder
 */
abstract class MenuTemplate<T : MenuBuilder<T>>(protected val lazy: Boolean = true) {
    ...
    // WARNING: note that using instance data in generateMenuBuilder or applyStaticElements will
    // result
    ...
    /**
     * Generate a menu builder used to create each menu. Static elements will immediately be
     * applied. This builder is cloned for each player that opens it, but is only generated once.
     *
     * Implementation specification: This method should NOT use any instance data not from super
     * class!
     *
    ...
    /**
     * Applies the dynamic elements to a cloned menu builder. This method is called every time a
     ...
     */
    fun openMenu(viewer: Player) =
        menuBuilder
            .clone()
            .also { applyDynamicElements(it, viewer) }
            .build()
            .apply { openMenu(viewer) }
}
