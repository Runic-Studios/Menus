 ...
 * @property menuBuilder The initial menu builder that the built generator will hold and make copies
 *   of to generate menus
 ...
 * @see com.runicrealms.menus.generator.MenuTemplate
 * @see com.runicrealms.menus.menu.Menu
 */
class MenuTemplateBuilder<T : MenuBuilder<T>>
@JvmOverloads
constructor(var menuBuilder: T, var lazy: Boolean = true) {
    ...
    fun build() =
        object : com.runicrealms.menus.generator.MenuTemplate<T>(lazy = lazy) {
            ...
            override fun applyStaticElements(menuBuilder: T): Unit = run {
                staticGenerator?.invoke(menuBuilder)
            }

            override fun applyDynamicElements(menuBuilder: T, viewer: Player): Unit = run {
                dynamicGenerator?.invoke(menuBuilder, viewer)
            }
        }
}
