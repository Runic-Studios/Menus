...
/**
 * Implementation of the Menu Builder allowing for menus of multiple pages. When setting items in
 * specific pages, <b>page numbers start from 0!</b>
 *
 * Note that while it is required to provide a default size (for newly created pages), each page can
 * be of different size. In addition page titles can be modified per page. In addition, page title
 * supports %pagenumber% which will be replaced with the number for that page.
 *
 ...
 * @property defaultSize The default size for new pages, size is between 9 and 54 and is a multiple
 *   of 9
...
internal constructor(
    ...
    var defaultTitle: TextComponent,
    val defaultSize: Int,
    ...
    /**
     * Map with entries specifying items that are "static" (identical) across all menu pages. The
     * key represents an inventory slot, the entry is a pair of the menu item, and an array of page
     * numbers to ignore, that should not contain the static item.
     */
    ...
    /**
     * Set an item with a specific page, slot, and MenuItem. This is unlike the default overloads
     * for addItem as they set items across all pages. Will expand the menu if the page number
     * provided does not yet exist.
     *
    ...
    /**
     * Set an item with a specific page, slot, and ItemStack. This is unlike the default overloads
     * for addItem as they set items across all pages. Will expand the menu if the page number
     * provided does not yet exist. The ItemStack icon is immediately used to generate a MenuItem
     * without any actions.
     *
     * @param slot The slot to insert the icon at
     * @param pageNumber The number of the page to insert the icon at
     ...
     * Set an item with a specific page, slot, and ItemStack. This is unlike the default overloads
     * for addItem as they set items across all pages. Will expand the menu if the page number
     * provided does not yet exist. The MenuItemBuilder is immediately used to build a MenuItem.
     *
     * @param slot The slot to insert the icon at
     * @param pageNumber The number of the page to insert the icon at
    ...
    /**
     * Set an item with a specific, slot, and MenuItem so that is identical across all pages. This
     * will apply to all pages that are created in the future as well. Excluded pages don't need to
     * exist yet, but on creation they will have the item not applied. The ItemStack icon is
     * immediately used to generate a MenuItem without any actions.
     *
     * @param slot The slot to insert the icon at
     * @param item The ItemStack icon to represent this slot
    ...
    fun setStaticItem(
        slot: Int,
        item: MenuItem,
        vararg excludePages: Int = intArrayOf(),
    ): PagedMenuBuilder {
        staticItems[slot] = Pair(item, excludePages)
        for ((index, page) in pages.withIndex()) if (!excludePages.contains(index))
            page.setItem(slot, item)
        return this
    }

    /**
     * Remove an item from a specific slot and page. If the slot is not populated, nothing will
     * happen.
     *
    ...
    fun removeItem(slot: Int, pageNumber: Int): PagedMenuBuilder {
        if (pageNumber !in 0 until pages.size)
            throw IllegalArgumentException("Invalid page size $pageNumber does not exist!")
        pages[pageNumber].items.remove(slot)
        return this
    }

    /**
     * Sets the inventory size for a page with a certain location in the menu. Pages can have
     * different sizes than the default for this menu.
     *
    ...
    /**
     * Sets the contents of all of the pages of this menu. This will immediately overwrite and
     * delete the current pages in this builder.
     *
    ...
    /**
     * Adds new pages to the menu. Will immediately populate them with the static items that have
     * been set.
     *
    ...
    fun addPage(amount: Int = 1, size: Int = defaultSize): PagedMenuBuilder {
        ...
        repeat(amount) {
            val pageItems =
                PagedMenu.Page(
                    defaultTitle.replaceText(
                        TextReplacementConfig.builder()
                            .matchLiteral(PAGE_NUMBER_PLACEHOLDER)
                            .replacement("${pages.size + 1}")
                            .build()
                    ) as TextComponent,
                    defaultSize,
                )
    ...
    /**
     * Sets how many pages should be in the menu. If we have too many pages, it deletes off the end.
    ...
    fun setMenuLength(length: Int): PagedMenuBuilder {
        if (length > pages.size) {
            ...
            repeat(pages.size - length) { pages.removeLast() }
        }
        return this
    }

    /**
     * Sets the page title for one specific page. Supports color codes and the page number
     * placeholder.
     *
     * @param title New title
     * @param pageNumbers Page numbers vararg of pages to apply to
     */
    fun setPageTitle(title: TextComponent, vararg pageNumbers: Int) {
        for (pageNumber in pageNumbers) {
            if (pageNumber >= pages.size)
                throw IllegalArgumentException(
                    "Page number $pageNumber does not yet exist in builder!"
                )
    ...
    override fun addBorder(
        borderItem: MenuItem,
        vararg borders: MenuBuilder.MenuBuilderBorder,
    ): PagedMenuBuilder {
        for (borderSide in borders) {
            when (borderSide) {
                // TODO needs updating when dropper menus exist
                MenuBuilder.MenuBuilderBorder.TOP ->
                    for (i in 0 until 9) setStaticItem(i, borderItem)
                MenuBuilder.MenuBuilderBorder.BOTTOM ->
                    for (i in 0 until 9) setStaticItem(size - i - 1, borderItem)
                MenuBuilder.MenuBuilderBorder.LEFT ->
                    for (i in 0 until size step 9) setStaticItem(i, borderItem)
                MenuBuilder.MenuBuilderBorder.RIGHT ->
                    for (i in 0 until size step 9) setStaticItem(i + 8, borderItem)
            }
        }
        return this
    }
    ...
    /**
     * Set the map of slots and pages to their actions on click. For use when cloning this menu
     * builder.
     *
     ...
     */
    fun setSlotActions(
        slotActions: MutableMap<Int, MutableMap<Int, MenuAction>>
    ): PagedMenuBuilder {
    ...
    override fun build(): PagedMenu {
        menuLoop@ for ((pageNumber, page) in slotActions) {
            ...
            pageLoop@ for ((slot, action) in page) {
                if (
                    slot < 0 ||
                        slot > pages[pageNumber].size ||
                        !pages[pageNumber].items.containsKey(slot)
                )
                    continue@pageLoop
    ...
    override fun clone(): PagedMenuBuilder {
        ...
        return PagedMenuBuilder(menusAPI, defaultTitle, defaultSize)
            .setPages(newPages)
