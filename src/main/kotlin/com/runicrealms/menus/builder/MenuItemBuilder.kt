 ...
 */
open class MenuItemBuilder @JvmOverloads constructor(material: Material, amount: Int = 1) :
    Cloneable {
     ...
     */
    constructor(material: Material, amount: Int, name: TextComponent) : this(material, amount) {
        setName(name)
    }
     ...
     * @return This builder for use in the builder pattern
     */
    fun setName(name: TextComponent): MenuItemBuilder {
        meta ?: throw IllegalStateException("Cannot set name for item with null meta!")
        meta!!.displayName(if (name.content().isEmpty()) Component.text(" ") else name)
        return this
    }
     ...
     * @return This builder for use in the builder pattern
     */
    fun setLore(vararg lore: TextComponent): MenuItemBuilder {
     ...
     * @return This builder for use in the builder pattern
     */
    fun setLore(lore: List<TextComponent>): MenuItemBuilder {
        meta ?: throw IllegalStateException("Cannot set lore for item with null meta!")
        meta!!.lore(lore.toList())
        return this
    }
     ...
     * @return This builder for use in the builder pattern
     */
    fun addLore(vararg lore: TextComponent): MenuItemBuilder {
        ...
        existingLore.addAll(lore.toList())
    ...
    fun addAttributeModifier(attribute: Attribute, modifier: AttributeModifier): MenuItemBuilder {
        meta
            ?: throw IllegalStateException(
                "Cannot add attribute modifiers for item with null meta!"
            )
     ...
     * @return This builder for use in the builder pattern
     */
    fun addAttributeModifiers(
        vararg modifiers: Pair<Attribute, AttributeModifier>
    ): MenuItemBuilder {
        meta
            ?: throw IllegalStateException(
                "Cannot add attribute modifiers for item with null meta!"
    ...
    fun setDamage(damage: Int): MenuItemBuilder {
        if (meta !is Damageable)
            throw IllegalStateException("Cannot apply damage for non Damageable meta!")
        (meta as Damageable).damage = damage
        return this
    }

    /**
     * Sets the ItemStack icon representing this menu item. This will also completely override the
     * current item meta and replace it with a new one! All changes will be lost.
     *
    ...
    /**
     * Sets the item meta for the ItemStack icon representing this menu item. Completely overrides
     * the previous item meta and any changes that were made to it.
     *
    ...
    /**
     * Sets if interactions should be blocked on this menu item. This decides if the viewer can drag
     * and drop to remove the icon from the menu. If not set then this will end up being the menu's
     * default.
     *
    ...
    /**
     * Clone this builder. The cloned builder will have the same refrences to the itemstack icon and
     * itemstack meta.
     *
