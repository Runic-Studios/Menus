package com.runicrealms.menus.util
...
internal object MenuUtil {
    ...
    /**
     * Returns the minimum required amount of inventory slots for a collection of slot numbers. This
     * value will be between 9 and 54, and will be a multiple of 9.
     *
    ...
    internal fun getMinSlots(slots: Set<Int>): Int {
        ...
        if (slots.size > 54 || lastSlot >= 54)
            throw IllegalArgumentException("Inventory slots must be between 0 and 53!")
        val minSlots = ceil(lastSlot / 9.0).toInt() * 9
        return max(minSlots, 9)
    }

    /**
     * Applies the Menu's defaultInteractionsBlocked to all MenuItems that haven't had their
     * interactionsBlocked set.
     *
     * Interactions blocked determines if a player can move an item in a menu, or if it will be
     * forced to remain static. Each menu item can have it set manually, but if none set the the
     * menu's default is applied.
     */
    internal fun applyDefaultInteractionsBlocked(
    ...
    /**
     * Gets the first empty page which has at least one slot empty in it. This uses the
     * MenuPageItems's size to determine if it is full, it will not increase a page's size to fit
     * more items. This method will add a new page to the list of pages if every page is full.
     *
     * @return First page with at least one empty slot
     */
    internal fun getFirstEmptyPage(
        pages: MutableList<PagedMenu.Page>,
        newPageDefaultTitle: TextComponent,
        newPageDefaultSize: Int,
    ): PagedMenu.Page {
    ...
    /**
     * Checks if a given menu size is valid, meaning it is greater than 0, less than or equal to 54,
     * and is a multiple of 9.
     *
